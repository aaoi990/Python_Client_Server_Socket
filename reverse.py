...
tls_extensions = {
    ...
    "extension_server_name": b"\x00\x00"
}
print(b'\x00#' == bytearray(b'\x00\x23'))
...
parser.add_argument(
    "-j", "--json", help="Output ndjson (either to file or stdout; overrides --output defaults to CSV)", action="store_true")
parser.add_argument(
    "-P", "--proxy", help="To use a SOCKS5 proxy, provide address:port.", type=str)
...
def map_extenstions(exts):
    # if "extension_server_name" not in exts:
    #     print("SNI is turned off - the proxy / openssl needed to enable this is too old to be viable.")
    for ext, value in exts.items():
        match ext:
            case "extended_master_secret":
                print(
                    "extended_master_secret is enable. EMS is enabled in OPENSSL versions 1.1.0 or later.", value)
                # OpenSSL supports Extended Master Secret extension starting with
                # OpenSSL 1.1.0, and there is no way to disable it. In nginx
                # Extended Master Secret extension will be configured if
                # running nginx with OpenSSL 1.1.0 or later.

            case "max_fragment_length":
                print("max_fragment_length", value)

            case "renegotiation_info":
                print("renegotiation_info", value)

            case "supported_groups":
                print("supported_groups", value)

            case "ec_point_formats":
                print("ec_point_formats", value)

            case "session_ticket":
                print("session_tickets are configured", value)
                # Session tickets can be diabled in nginx by setting
                # ssl_session_tickets off;
                # ssl_session_cache off;
                # This results in:
                # SSL_CTX_set_session_cache_mode(ctx, SSL_SESS_CACHE_OFF)
                # SSL_CTX_set_options(ctx, SSL_OP_NO_TICKET)
                # in the openssl library so neither stateful nor stateless tickets should be
                # used.

            case "signature_algorithms":
                print("signature_algorithms", value)

            case "psk_key_exchange_modes":
                print("psk_key_exchange_modes", value)

            case _:
                print("#########################", ext, value)


def parse_extentions(extenstions):
    mapping = {}
    values = tls_extensions.values()
    for count, ext in enumerate(extenstions, 0):
        if ext[0] in values:
            mapping[[k for k, v in tls_extensions.items() if v == ext[0]]
                    [0]] = ext[1]

    map_extenstions(mapping)
...
def get_extensions(jarm_details):
    ...
    max_fragment_length = b"\x00\x01\x00\x01\x02"
...
def extension_server_name(host):
    # host is the ip address or domain name provided on the command line
    ext_sni = b"\x00\x00"
...
def read_packet(data, jarm_details):
    try:
        ...
        # Check for server hello
            ...
            jarm += "|"
            # Extract extensions
            extensions, ext_mapping = (extract_extension_info(
                data, counter, server_hello_length))
            jarm += extensions
            return jarm, ext_mapping
        else:
            return "|||", []
...
def extract_extension_info(data, counter, server_hello_length):
    try:
        ...
        alpn = find_alpn(b"\x00\x10", types, values)
        ext_mapping = list(zip(types, values))
        result += str(alpn)
        result += "|"
...
def find_alpn(ext_type, types, values):
    iter = 0
...
def find_extension(ext_type, types, values):
    for count, type in enumerate(types, 0):
        if types[count] == ext_type:
            print(count, type)
    # iter = 0
    # # For the APLN extension, grab the value in ASCII
    # if ext_type == b"\x00\x10":
...
def main():
    ...
    while iterate < len(queue):
        ...
        if server_hello == "TIMEOUT":
            jarm = "|||,|||,|||,|||,|||,|||,|||,|||,|||,|||"
            break
        ans, ext_mapping = read_packet(server_hello, queue[iterate])
        mapping += ext_mapping
        jarm += ans
        iterate += 1
    ...
    # Fuzzy hash
    new_list = []
    a = [new_list.append(item) for item in mapping if item not in new_list]
    parse_extentions(new_list)
    result = jarm_hash(jarm)
    # Write to file
    if args.output:
        if ip != None:
            if args.json:
                file.write('{"host":"' + destination_host +
                           '","ip":"' + ip + '","result":"' + result + '"')
            else:
    ...
    # Print to STDOUT
        ...
        if ip != None:
            if args.json:
                sys.stdout.write('{"host":"' + destination_host +
                                 '","ip":"' + ip + '","result":"' + result + '"')
            else:
